...
async function gradeTheoryAnswers(
	...
	return candidateData.theory_answers.reduce(async (accPromise: any, studentAnswer: string, index) => {
		...
		const context = courseDetails.context .map((psAnsIt: string) => `- ${psAnsIt}\n`)
		.join( "\n\n--------------------------------------------------------------------------------------\n\n",);
		const gradingRubric = courseDetails.rubric;

		const gptPrompt = getGPTPrompt(school_name,course.name,course.course_code,totalScoreForQuestion,question,studentAnswer,context,
			gradingRubric,
		);
		
		const openai = new OpenAI({ apiKey: `${process.env.OPEN_AI_KEY}` });
		const completion: any = await openai.chat.completions.create({messages: [{ role: "user", content: gptPrompt }],model: "gpt-4o-mini",
			temperature: 0.5,
		});

		const gradeResult = getGradeResult(completion);
		acc.theoryScore += gradeResult!.totalScore;
		acc.theoryGradeAndSummary.push({question: question,answer: studentAnswer,mark: gradeResult!.totalScore,reason: gradeResult!.summary,});
		return acc;

	}, Promise.resolve({ theoryScore: 0, theoryGradeAndSummary: [
		{ question: '', answer: '', mark: '', reason: '' },
	] }));
}
